...
async function RenderMoviesByCategory({ page, category }: { page: number, category: string }) {
...
async function RenderMoviesBySort({ page, sortBy }: { page: number, sortBy: string }) {
  const movies = await getSortedMovies(page, sortBy);
  const imageBaseUrl = 'https://image.tmdb.org/t/p/w500';
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-x-20 gap-y-10">
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24 bg-gradient-to-br from-[#3079a2] to-[#681a74] text-white">
      ...
      <Suspense fallback={<div>Loading...</div>}>
        {sortBy ? (
          <RenderMoviesBySort page={page} sortBy={sortBy} />
        ) : (
          <RenderMoviesByCategory page={page} category={category || "popular"} />
        )}
      </Suspense>
      <PaginationControls page={page} category={category || "popular"} sortBy={sortBy} />
    </main >
  );
}
